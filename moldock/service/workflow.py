...
class DockingFlow(FlowSpec):
    ...
    receptor_path = Parameter('receptor_path', type=str, help='Path to receptor pdb file')
    ...
    def start(self):
        ...
        if not (os.path.exists(self.receptor_path) and os.path.exists(self.ligand_path)):
            raise ValueError(F'Either receptor or ligand specified not available at locations.')     
